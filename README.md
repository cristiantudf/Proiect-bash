...
A lightweight and efficient Bash script designed for user administration, featuring registration, authentication, logout, and report generation capabilities.
...
All user data is persisted in a local `utilizatori.csv` file with the following structure: ID, Username, Email, PasswordHash, LastLogin. User IDs are generated from a SHA-256 hash of the current timestamp. Passwords are never stored in plain text — only their SHA-256 hash is saved.
